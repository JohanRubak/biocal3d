...
OUTPUT_ROOT = DATA_ROOT / "_plaque_detection_benchmark_v4"
...
def build_texture_index(root: Path) -> Dict[Tuple[str, int, int, int], List[Path]]:
    """Index raw scanner texture images by (scanner, group, time, specimen).

    Only non-underscore top-level scanner folders are searched. This avoids
    accidentally selecting QC PNGs from _processed_ROI or analysis outputs.
    """
    ...
    for top in root.iterdir():
        if not top.is_dir() or top.name.startswith("_"):
            continue
        scanner = _canonical_scanner_from_name(top.name)
        if scanner is None:
            continue

        for image_path in top.rglob("*"):
            if not image_path.is_file() or image_path.suffix.lower() not in TEXTURE_EXTENSIONS:
                continue
            sample_folder = _nearest_sample_ancestor(image_path, root)
            if sample_folder is None:
                continue
            parsed = parse_sample_name(sample_folder.name)
            if parsed is None:
    ...
    for key, paths in index.items():
        paths.sort(key=_texture_candidate_score, reverse=True)
    return index
...
def save_prediction_example_figure(scan: ScanData, pred_by_method: dict):
    """Save original ROI beside binary predicted plaque overlays for all methods."""
    ...
    n = len(scan.vertices)
    r = rng_for(scan.scanner, scan.sample_name, "prediction_example")
    if n > EXAMPLE_MAX_POINTS:
        idx = r.choice(n, size=EXAMPLE_MAX_POINTS, replace=False)
    else:
        idx = np.arange(n)
    ...
    # Binary overlays: preserve the original underneath and mark predicted
    # plaque-like vertices in red so spatial localization can be judged directly.
    for j, method in enumerate(METHODS, start=1):
        ax = axes[j]
        pred = pred_by_method[method]
        mask = pred["mask"][idx].astype(bool)

        ax.scatter(
            xy[:, 0], xy[:, 1], c=rgb, s=5, linewidths=0, alpha=0.35
        )
        if np.any(mask):
            ax.scatter(
                xy[mask, 0], xy[mask, 1],
                c="red", s=6, linewidths=0, alpha=0.75,
            )
    ...
    fig.suptitle(
        ...
        "Red overlay = predicted plaque-like area",
        fontsize=14,
    )
    fig.tight_layout(rect=(0, 0, 1, 0.94))
    fig.savefig(out_dir / f"{scan.sample_name}_prediction_example.png", dpi=200)
    plt.close(fig)
