...
The loader supports either direct Lab values or RGB vertex colours. For
LABscanner, roi.vtp files that contain TCoords are automatically textured from
the corresponding raw JPG/PNG found under DATA_ROOT/LABscanner. If your
...
OUTPUT_ROOT/
  ...
  prediction_examples/<scanner>/<sample>.png
      Original coloured ROI + binary plaque overlays for all six methods.
...
OUTPUT_ROOT = DATA_ROOT / "_plaque_detection_benchmark_v5"
TABLE_ROOT = OUTPUT_ROOT / "tables"
PRED_ROOT = OUTPUT_ROOT / "predictions"
QC_ROOT = OUTPUT_ROOT / "qc"
MODEL_ROOT = OUTPUT_ROOT / "fold_models"
EXAMPLE_ROOT = OUTPUT_ROOT / "prediction_examples"
...
# Easy-to-read binary prediction examples. By default these are produced for
# every Partial scan: original ROI colour + one plaque-mask overlay per method.
SAVE_PREDICTION_EXAMPLES = True
EXAMPLE_ONLY_PARTIAL = True
EXAMPLE_MAX_POINTS = 22000

# LABscanner texture mapping. VTK/PLY UV coordinates conventionally use v=0 at
# the bottom, while image arrays use row 0 at the top, hence the vertical flip.
TEXTURE_FLIP_V = True
...
# =============================================================================
# HELPERS: RAW TEXTURE INDEX (LABscanner)
# =============================================================================


def _canonical_scanner_from_name(name: str) -> Optional[str]:
    compact = name.lower().replace("_", "").replace("-", "").replace(" ", "")
    if "trios3" in compact:
        return "TRIOS3"
    if "trios5" in compact:
        return "TRIOS5"
    if "itero" in compact:
        return "iTERO"
    if "labscanner" in compact:
        return "LABscanner"
    return None


def _texture_candidate_score(path: Path) -> Tuple[int, int, str]:
    """Rank likely scanner texture images ahead of unrelated photos."""
    ...
    if path.suffix.lower() in {".jpg", ".jpeg"}:
        score += 10
    if "qc" in name or "overview" in name or "figure" in name:
        score -= 100
    try:
        size = int(path.stat().st_size)
    except Exception:
        size = 0
    return score, size, str(path).lower()


def _scanner_from_full_path(path: Path) -> Optional[str]:
    """Infer scanner from ANY folder/file component in a path."""
    for part in reversed(path.parts):
        scanner = _canonical_scanner_from_name(part)
        if scanner is not None:
            return scanner
    compact = str(path).lower().replace("_", "").replace("-", "").replace(" ", "")
    if "labscanner" in compact:
        return "LABscanner"
    if "trios3" in compact:
        return "TRIOS3"
...
def _sample_from_full_path(path: Path):
    """Recover BCG group/time/specimen from any ancestor or the full path."""
    for part in reversed(path.parts):
        parsed = parse_sample_name(part)
        if parsed is not None:
            return parsed
    m = SAMPLE_RE.search(str(path))
    if m is None:
        return None
    group = int(m.group("group"))
    time_idx = int(m.group("time"))
    specimen_num = int(m.group("sample"))
    physical_id = f"G{group}_S{specimen_num}"
    stage = stage_from_group_time(group, time_idx)
    return group, time_idx, specimen_num, physical_id, stage


def build_texture_index(root: Path) -> Dict[Tuple[str, int, int, int], List[Path]]:
    """Robustly index raw texture images throughout DATA_ROOT.

    Previous versions assumed the raw scanner folder was an immediate child of
    DATA_ROOT.  This version searches the full tree, but deliberately ignores
    generated/processed folders whose path components start with ``_``.
    Scanner and sample IDs may occur anywhere in the ancestor path.
    """
    index: Dict[Tuple[str, int, int, int], List[Path]] = {}
    if not root.exists():
        return index
    ...
    for image_path in root.rglob("*"):
        ...
        # Ignore generated analysis/preprocessing products such as
        # _processed_ROI, _raw_color_analysis and benchmark output folders.
        if any(part.startswith("_") for part in rel.parts[:-1]):
            continue

        scanner = _scanner_from_full_path(image_path)
        if scanner is None:
            continue

        parsed = _sample_from_full_path(image_path)
        if parsed is None:
            continue
        group, time_idx, specimen_num, _, _ = parsed
        key = (scanner.lower(), group, time_idx, specimen_num)
        index.setdefault(key, []).append(image_path)

    for key, paths in index.items():
        # Remove duplicates while preserving deterministic ranking.
        unique = list(dict.fromkeys(paths))
        unique.sort(key=_texture_candidate_score, reverse=True)
        index[key] = unique
    return index


def choose_texture_path(
    texture_index: Dict[Tuple[str, int, int, int], List[Path]],
    scanner: str,
    group: int,
    time_idx: int,
    specimen_num: int,
) -> Optional[Path]:
    paths = texture_index.get((scanner.lower(), group, time_idx, specimen_num), [])
    return paths[0] if paths else None


def read_texture_rgb(path: Path) -> np.ndarray:
    """Read a texture image as float RGB in [0, 1] without extra dependencies."""
    img = plt.imread(path)
    img = np.asarray(img)
    if img.ndim == 2:
        img = np.repeat(img[..., None], 3, axis=2)
    if img.ndim != 3 or img.shape[2] < 3:
        raise ValueError(f"Texture image is not RGB-like: {path} shape={img.shape}")
...
def sample_texture_at_uv(texture_rgb: np.ndarray, uv: np.ndarray) -> np.ndarray:
    """Bilinearly sample an RGB texture at VTK/PLY UV coordinates."""
    uv = np.asarray(uv, dtype=float)
    if uv.ndim != 2 or uv.shape[1] < 2:
        raise ValueError(f"TCoords must be N x 2, got {uv.shape}")
    ...
    return np.clip(top * (1.0 - wy) + bottom * wy, 0.0, 1.0)
...
def lab_to_display_rgb(lab: np.ndarray) -> np.ndarray:
    """Convert Lab to display RGB only for figures when source RGB is absent."""
...
def _coerce_faces_array(faces: np.ndarray) -> Optional[np.ndarray]:
    """Return triangular faces without slow Python cell-by-cell parsing.

    Supported inputs:
      * M x 3 (or wider) triangle arrays
      * PyVista/VTK flat triangle format: [3, i, j, k, 3, i, j, k, ...]

    Non-triangular flattened connectivity is intentionally returned as None.
    VTP meshes are triangulated safely in ``load_from_vtp`` before this helper
    is called, so there is no reason to walk a potentially huge/corrupt flat
    array in Python.
    """
    if faces is None:
        return None
    ...
    if arr.ndim == 2 and arr.shape[1] >= 3:
        out = np.asarray(arr[:, :3], dtype=np.int64)
        return out if len(out) else None

    arr = np.asarray(arr, dtype=np.int64).ravel()

    # Fast path for the standard VTK/PyVista triangle layout.
    if arr.size % 4 == 0:
        cells = arr.reshape(-1, 4)
        if np.all(cells[:, 0] == 3):
            return np.asarray(cells[:, 1:4], dtype=np.int64)

    # Do not fall back to a Python while-loop here.  Besides being very slow
    # for large connectivity arrays, malformed cell sizes can make that loop
    # effectively non-terminating.  Callers can still use vertex-fraction
    # coverage when triangle connectivity is unavailable.
    return None
...
def load_from_vtp(
    ...
    # ``mesh.faces`` is a flattened VTK connectivity array.  Parsing arbitrary
    # polygons one cell at a time in Python can become extremely slow and can
    # hang on malformed connectivity.  First use the cheap vectorized triangle
    # path; if the mesh contains polygons, let VTK triangulate them in compiled
    # code and then read the regular triangle connectivity.
    faces = _coerce_faces_array(np.asarray(mesh.faces))
    if faces is None and mesh.n_cells > 0:
        try:
            tri_mesh = mesh.triangulate(inplace=False)
            tri_faces = _coerce_faces_array(np.asarray(tri_mesh.faces))
            if tri_faces is not None:
                mesh = tri_mesh
                faces = tri_faces
        except Exception as exc:
            # Face information is optional for detection.  If triangulation
            # fails, retain the point data and later report vertex-fraction
            # coverage instead of physical mesh area for this scan.
            print(f"    warning: could not triangulate {path.name}: {exc}")
            faces = None
    ...
    # LABscanner stores colour as UV texture coordinates rather than direct
    # per-vertex RGB. Reconstruct the ROI vertex colours from the original JPG.
    if rgb is None and "TCoords" in mesh.point_data:
        if texture_path is None:
            raise ValueError(
                f"TCoords found in {path}, but no matching raw texture image was "
                "found. Expected a JPG/PNG beneath the raw scanner folder for "
                f"sample {path.parent.name}."
            )
        uv = np.asarray(mesh.point_data["TCoords"], dtype=float)
        texture_rgb = read_texture_rgb(texture_path)
        rgb = sample_texture_at_uv(texture_rgb, uv)
    ...
    if lab is None:
        raise ValueError(
            f"No Lab/RGB could be recovered from {path}. "
            f"Available point arrays: {list(mesh.point_data.keys())}. "
            f"Texture candidate: {texture_path}"
        )
    return lab, rgb, vertices, faces
...
def load_scan(
    scanner: str,
    folder: Path,
    texture_index: Optional[Dict[Tuple[str, int, int, int], List[Path]]] = None,
) -> ScanData:
    ...
    texture_path = choose_texture_path(
        texture_index, scanner, group, time_idx, specimen_num
    )
    ...
    # LABscanner ROI files contain TCoords and should be loaded from VTP first
    # so the raw texture can be mapped correctly to each ROI vertex.
    if scanner.lower() == "labscanner":
        for vtp_path in vtp_candidates:
            try:
                loaded = load_from_vtp(vtp_path, texture_path=texture_path)
...
def surface_faces_for_plot(scan: ScanData, xy: np.ndarray) -> np.ndarray:
    """Return triangles covering the ROI for continuous filled-surface plots.

    Prefer the real mesh connectivity. If face connectivity is unavailable,
    triangulate the nearly planar ROI in its PCA projection. This fallback is
    used for VISUALIZATION ONLY; physical area calculations still use the real
    3D mesh faces when available.
    """
    if scan.faces is not None:
        faces = np.asarray(scan.faces, dtype=int)
        if (
            faces.ndim == 2
            and faces.shape[1] == 3
            and len(faces) > 0
            and np.min(faces) >= 0
            and np.max(faces) < len(xy)
        ):
            return faces
    ...
    try:
        ...
        # Avoid the rare very-long triangle spanning a gap at the ROI edge.
        # Keep triangles whose longest edge is within a robust limit derived
        # from the observed triangulation itself.
        pts = xy[faces]
        ...
        med = np.median(longest[np.isfinite(longest)])
        if np.isfinite(med) and med > 0:
            keep = longest <= 4.0 * med
            if np.any(keep):
                faces = faces[keep]
        return faces
    except Exception:
...
def _autoscale_surface_axis(ax, xy: np.ndarray):
    if len(xy) == 0:
        return
...
def draw_rgb_surface(ax, xy: np.ndarray, faces: np.ndarray, rgb: np.ndarray,
                     alpha: float = 1.0):
    """Render a fully filled triangulated ROI using mean vertex RGB per face."""
    if len(faces) == 0:
        ax.scatter(xy[:, 0], xy[:, 1], c=rgb, s=5, linewidths=0)
        _autoscale_surface_axis(ax, xy)
        return None

    face_rgb = np.clip(np.mean(rgb[faces], axis=1), 0.0, 1.0)
    coll = PolyCollection(
        xy[faces],
        facecolors=face_rgb,
        edgecolors="none",
...
def draw_score_surface(ax, xy: np.ndarray, faces: np.ndarray, score: np.ndarray):
    """Render a continuous, fully covered face-level score map."""
    if len(faces) == 0:
        sc = ax.scatter(xy[:, 0], xy[:, 1], c=score, s=5, linewidths=0)
        _autoscale_surface_axis(ax, xy)
        return sc

    face_score = np.mean(score[faces], axis=1)
    finite = face_score[np.isfinite(face_score)]
    if len(finite):
...
def draw_plaque_overlay(ax, xy: np.ndarray, faces: np.ndarray, rgb: np.ndarray,
                        vertex_mask: np.ndarray):
    """Render original textured surface with plaque-positive faces fully filled.

    A triangle is called plaque-like when at least two of its three vertices
    are plaque-like. This is the same majority rule used by the area estimate.
    """
    draw_rgb_surface(ax, xy, faces, rgb, alpha=0.55)

    if len(faces) == 0:
        mask = np.asarray(vertex_mask, dtype=bool)
        if np.any(mask):
            ax.scatter(xy[mask, 0], xy[mask, 1], c="red", s=6,
                       linewidths=0, alpha=0.75)
        return

    face_plaque = np.mean(vertex_mask[faces], axis=1) >= 0.5
    if np.any(face_plaque):
        overlay = PolyCollection(
            xy[faces[face_plaque]],
            facecolors="red",
            edgecolors="none",
            linewidths=0,
            alpha=0.72,
...
def save_prediction_npz(scan: ScanData, pred_by_method: dict):
    ...
    out_path = out_dir / f"{scan.sample_name}.npz"

    payload = {
        "vertices": scan.vertices,
        "lab": scan.lab,
        "physical_id": np.array(scan.physical_id),
        "stage": np.array(scan.stage),
    }
    if scan.rgb is not None:
        payload["rgb"] = scan.rgb
    if scan.faces is not None:
        payload["faces"] = scan.faces

    for method, result in pred_by_method.items():
        payload[f"{method}_score"] = result["score"].astype(np.float32)
        payload[f"{method}_mask"] = result["mask"].astype(np.uint8)
        payload[f"{method}_threshold"] = np.array(result["threshold"])
...
def save_qc_figure(scan: ScanData, pred_by_method: dict):
    ...
    # Full-surface plots: do NOT subsample vertices, because doing so destroys
    # triangle connectivity and produces the sparse point maps seen in v4.
    xy = project_vertices_2d(scan.vertices)
    faces = surface_faces_for_plot(scan, xy)
    display_rgb = scan.rgb if scan.rgb is not None else lab_to_display_rgb(scan.lab)
    ...
    ax = axes[0]
    draw_rgb_surface(ax, xy, faces, display_rgb)
    ax.set_title("Original ROI colour\nfilled triangulated surface")

    for j, method in enumerate(METHODS, start=1):
        ax = axes[j]
        result = pred_by_method[method]
        artist = draw_score_surface(ax, xy, faces, result["score"])
...
def save_prediction_example_figure(scan: ScanData, pred_by_method: dict):
    """Save FULLY FILLED original ROI and binary plaque overlays for all methods."""
    if EXAMPLE_ONLY_PARTIAL and scan.stage != "partial":
        return

    out_dir = EXAMPLE_ROOT / scan.scanner
    out_dir.mkdir(parents=True, exist_ok=True)
    ...
    # Full original textured ROI.
    ax = axes[0]
    draw_rgb_surface(ax, xy, faces, display_rgb)
    ax.set_title("Original ROI colour\nfilled surface")

    # Full face overlays: predicted plaque is shown as continuous red regions,
    # not as red vertex dots.
    for j, method in enumerate(METHODS, start=1):
        ax = axes[j]
        pred = pred_by_method[method]
        mask = np.asarray(pred["mask"], dtype=np.uint8)
        draw_plaque_overlay(ax, xy, faces, display_rgb, mask)

        cov = float(pred["coverage_pct"])
        area = float(pred.get("plaque_area", np.nan))
        if np.isfinite(area):
            subtitle = f"{cov:.1f}% | {area:.2f} mm²"
        else:
            subtitle = f"{cov:.1f}% | area unavailable"

        short = method.split("_", 1)[0]
        label = {
            "M1": "a* threshold",
            "M2": "clean distance",
            "M3": "Lab reference",
        ...
        ax.set_title(f"{short} {label}\nPredicted plaque-like: {subtitle}")

    for ax in axes[panels:]:
        ax.axis("off")

    texture_note = f" | texture={scan.texture_path.name}" if scan.texture_path else ""
    fig.suptitle(
        f"{scan.scanner} | {scan.sample_name} | {scan.stage} | {scan.physical_id}{texture_note}\n"
        "Red filled surface = predicted plaque-like area",
        fontsize=14,
    )
    fig.tight_layout(rect=(0, 0, 1, 0.94))
    fig.savefig(out_dir / f"{scan.sample_name}_prediction_example.png", dpi=220)
    plt.close(fig)
...
def main():
    for folder in [OUTPUT_ROOT, TABLE_ROOT, PRED_ROOT, QC_ROOT, MODEL_ROOT, EXAMPLE_ROOT]:
    ...
    texture_index = build_texture_index(DATA_ROOT)
    lab_tex = sum(1 for k in texture_index if k[0] == "labscanner")
    print(f"Raw texture index: {sum(len(v) for v in texture_index.values())} images "
          f"across {len(texture_index)} scans; LABscanner scans with texture={lab_tex}")
    ...
    for i, (scanner, folder) in enumerate(discovered, 1):
        try:
            scan = load_scan(scanner, folder, texture_index=texture_index)
            if scan.stage.startswith("unknown"):
    ...
    inventory = pd.DataFrame([{
        ...
        "texture_path": str(s.texture_path) if s.texture_path else "",
            ...
            for scan in test_scans:
                ...
                for method, detector in detectors.items():
                    ...
                    pred_by_method[method] = {
                        ...
                        "plaque_area": summary["plaque_like_area"],
                        "total_roi_area": summary["total_roi_area"],
                        "area_method": summary["area_method"],
                    }
                ...
                if SAVE_PREDICTION_EXAMPLES:
                    save_prediction_example_figure(scan, pred_by_method)
    ...
    if SAVE_PREDICTION_EXAMPLES:
        print(f"Prediction examples: {EXAMPLE_ROOT}")
    print("=" * 80)
