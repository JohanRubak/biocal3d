...
SAMPLE_RE = re.compile(
    r"BCG(?P<group>\d+)T(?P<time>\d+)-(?P<sample>\d+)",
    re.IGNORECASE,
)
...
def discover_sample_folders(root: Path) -> List[Tuple[str, Path]]:
    """Return (scanner, sample_folder) pairs."""
    found = []
    if not root.exists():
        raise FileNotFoundError(f"DATA_ROOT does not exist: {root}")

    # The scanner is taken as the first directory beneath DATA_ROOT.
    for scanner_dir in sorted([p for p in root.iterdir() if p.is_dir()]):
        if scanner_dir.name.startswith("_"):
            continue
        scanner = scanner_dir.name
        for p in scanner_dir.rglob("*"):
            if not p.is_dir():
                continue
            if SAMPLE_RE.search(p.name):
                if (p / "geometry.npz").exists() or (p / "roi.vtp").exists():
                    found.append((scanner, p))
    return found
...
def load_scan(scanner: str, folder: Path) -> ScanData:
    ...
    loaded = None
    npz_path = folder / "geometry.npz"
    if npz_path.exists():
        loaded = load_from_npz(npz_path)

    if loaded is None:
        vtp_path = folder / "roi.vtp"
        if not vtp_path.exists():
            raise FileNotFoundError(f"No usable geometry.npz or roi.vtp in {folder}")
...
def main():
    ...
    if not scans:
        raise RuntimeError("No usable scans loaded. Check DATA_ROOT and colour arrays.")
